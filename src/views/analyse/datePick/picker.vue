<template>
  <div
    ...
    v-if="relativeTimeData.label">
    <input
      :value="relativeTimeData.label"
      :readonly="true"
      @change="emit('input', relativeTimeData.value)"
      @focus="handleFocus"
      class="el-time-input">
  </div>
  <div
...
export default {
  ...
  computed: {
    ...
    displayValue() {
      if (this.relativeTimeData.label) {
        return '';
      }
  ...
  created() {
    ...
    if (!Array.isArray(this.value)) {
      this.relativeTimeData = this.timeMap.find(item => item.value == this.value);
    }
  },
  ...
  methods: {
    ...
    mountPicker() {
      ...
      if (!Array.isArray(this.value)) {
        this.picker.setTimeValue(this.value);
      }
    },
